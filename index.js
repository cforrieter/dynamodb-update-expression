...
var updateExpressionGenerator = function(compareResult, options, path, excludeFields) {
  var request = {
    UpdateExpression: '',
...
var removeExpressionGenerator = function(original, removes, compareResult, path, itemUniqueId) {
  var request = {
    UpdateExpression: '',
    ExpressionAttributeNames: {},
    ExpressionAttributeValues: {},
  ...
  updateList.forEach(function(expr) {
    ...
    var splittedByDotPropName, propNameExpressionName, propNameExpressionValue

    if (expr.dataType !== 'list') {
    } else {
      var value = null
      // Remove any elements that specified in removes json
      if (typeof _.get(original, expr.name)[0] === 'object' || typeof _.get(removes, expr.name)[0] === 'object') {
...
exports.generateRemoveExpression = function(original, removes, itemUniqueId) {
  return removeExpressionGenerator(original, removes, deepDiffMapper.map(removes, original), null, itemUniqueId)
}
